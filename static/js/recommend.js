$(function () {
  ...
  const source = document.getElementById("autoCompleteMovies");
  ...
  $(".movie-button").on("click", function () {
    var my_api_key = "e2b4330ad1b7d8cf3b4a0b4adaf7a403";
    var title = $(".movie").val();
    if (title == "") {
      ...
      if (my_movie.cast.length > 0) {
        ...
        for (var my_cast in top_cast) {
          ...
          if (my_movie.cast[my_cast].profile_path) {
            ...
            cast_profiles.push("static/img/default.jpg");
          }
        }
      }
    },
    ...
    success: function (recommend) {
      for (var recs in recommend.results) {
        ...
        if (recommend.results[recs].poster_path) {
          ...
          rec_posters.push("static/img/default.jpg");
        }
      }
    },
    error: function (error) {
